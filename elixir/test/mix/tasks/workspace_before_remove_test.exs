defmodule Mix.Tasks.Workspace.BeforeRemoveTest do
  ...
  test "uses current branch for lookup when branch option is omitted" do
    with_fake_gh_and_git(
      ...
      if [ "$1" = "remote" ] && [ "$2" = "get-url" ] && [ "$3" = "origin" ]; then
        printf 'git@github.com:example/march.git\n'
        exit 0
      fi
      ...
      fn log_path ->
        ...
        assert log =~
                 "pr list --repo example/march --head feature/workpad --state open --json number --jq .[].number"

        assert log =~ "pr close 101 --repo example/march"
        assert log =~ "pr close 102 --repo example/march"
      end
    )
  end
  ...
  test "closes open pull requests for the branch and tolerates close failures" do
    with_fake_gh(fn log_path ->
      ...
      {output, error_output} =
        capture_task_output(fn ->
          BeforeRemove.run(["--repo", "owner/repo", "--branch", "feature/workpad"])
        end)
      ...
      assert log =~ "pr list --repo owner/repo --head feature/workpad --state open --json number --jq .[].number"
      assert log =~ "pr close 101 --repo owner/repo"
      assert log =~ "pr close 102 --repo owner/repo"
      ...
      fn log_path ->
        error_output =
          capture_io(:stderr, fn ->
            Mix.Task.reenable("workspace.before_remove")
            BeforeRemove.run(["--repo", "owner/repo", "--branch", "feature/no-output"])
          end)

        assert error_output =~ "Failed to close PR #102 for branch feature/no-output: exit 17"
        refute error_output =~ "output="
        log = File.read!(log_path)
        assert log =~ "pr list --repo owner/repo --head feature/no-output --state open --json number --jq .[].number"
        assert log =~ "pr close 102 --repo owner/repo"
      end
    )
  end
  ...
  test "no-ops when PR list fails for current branch" do
    with_fake_gh(
      ...
      fn log_path ->
        output =
          capture_io(fn ->
            BeforeRemove.run(["--repo", "owner/repo", "--branch", "feature/list-fails"])
          end)
        ...
        assert log =~
                 "pr list --repo owner/repo --head feature/list-fails --state open --json number --jq .[].number"
