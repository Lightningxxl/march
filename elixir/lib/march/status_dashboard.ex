defmodule March.StatusDashboard do
  @moduledoc """
  ...
  def init(opts) do
    ...
    enabled = resolve_override(enabled_override, Config.observability_enabled?())
  ...
  def render_offline_status do
    content =
      [
        colorize("╭─ MARCH CONSOLE", @ansi_bold),
  ...
  defp refresh_runtime_config(%__MODULE__{} = state) do
    %{
      state
      | enabled: resolve_override(state.enabled_override, Config.observability_enabled?()),
  ...
  defp format_snapshot_content(snapshot_data, tps, terminal_columns_override \\ nil) do
    case snapshot_data do
      {:ok, %{running: running, retrying: retrying, codex_totals: codex_totals} = snapshot} ->
        ...
        task_source_line = format_task_source_line()
  ...
  defp format_task_source_line do
    source_part =
    ...
    colorize("│ Task Source: ", @ansi_bold) <> source_part
  end
