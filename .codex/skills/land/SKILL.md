...
5. If conflicts exist, use the `pull` skill to fetch/merge the canonical base branch and
...
- If CI pushes an auto-fix commit (authored by GitHub Actions), it does not
  ...
  the current base branch if needed, add a real author commit, and force-push to retrigger
  CI, then restart the checks loop.
- If all jobs fail with corrupted pnpm lockfile errors on the merge commit, the
  remediation is to fetch the latest base branch, merge, force-push, and rerun CI.
